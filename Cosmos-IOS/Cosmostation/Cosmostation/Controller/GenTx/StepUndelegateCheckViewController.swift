...
class StepUndelegateCheckViewController: BaseViewController, PasswordViewDelegate{
    ...
    func onBroadcastTxV1(_ auth: Auth_V1) {
        DispatchQueue.global().async {
            ...
            let stdTx = Signer.genSignedUndelegateTxV1(auth.getAddress(), auth.getAccountNumber(), auth.getSequenceNumber(),
                                                       self.pageHolderVC.mTargetValidator_V1!.operator_address!, self.pageHolderVC.mToUndelegateAmount!, self.pageHolderVC.mFee!, self.pageHolderVC.mMemo!,
            ...
            DispatchQueue.main.async(execute: {
                ...
                request.responseJSON { response in
                    ...
                    case .failure(let error):
                        if(SHOW_LOG) { print("UnDelegate error ", error) }
                        if (response.response?.statusCode == 500) {
                            txResult["net_error"] = 500
                        }
                    }
