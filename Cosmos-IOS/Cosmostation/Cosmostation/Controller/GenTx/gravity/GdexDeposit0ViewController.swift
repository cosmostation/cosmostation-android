//
//  GdexDeposit0ViewController.swift
//  Cosmostation
//
...
class GdexDeposit0ViewController: BaseViewController, UITextFieldDelegate {
    ...
    func onInitView() {
        let txFeeAmount = WUtils.getEstimateGasFeeAmount(chainType!, LIQUIDITY_MSG_TYPE_JOIN_POOL, 0)
        ...
        if (coin0Denom == COSMOS_MAIN_DENOM) {
        ...
        if (coin1Denom == COSMOS_MAIN_DENOM) {
            available1MaxAmount = available1MaxAmount.subtracting(txFeeAmount)
        }
        
        coin0Decimal = WUtils.getCosmosCoinDecimal(coin0Denom)
        coin1Decimal = WUtils.getCosmosCoinDecimal(coin1Denom)
        
        WUtils.DpCosmosTokenImg(inputCoin0Img, coin0Denom)
        WUtils.DpCosmosTokenName(inputCoin0Name, coin0Denom)
        WUtils.DpCosmosTokenImg(inputCoin1Img, coin1Denom)
        WUtils.DpCosmosTokenName(inputCoin1Name, coin1Denom)
    ...
    @IBAction func onClickNext(_ sender: UIButton) {
        if (isValiadAmount()) {
            ...
            let coin0Amount = NSDecimalNumber.init(string: selectedPoolManager.reserve.filter {$0.denom == coin0Denom }.first?.amount)
            let expectedLpTokenAmount = pool0Amount.multiplying(by: totalShare).dividing(by: coin0Amount, withBehavior: WUtils.handler18)
            pageHolderVC.mLPCoin = Coin.init(selectedPool.poolCoinDenom, expectedLpTokenAmount.stringValue)
