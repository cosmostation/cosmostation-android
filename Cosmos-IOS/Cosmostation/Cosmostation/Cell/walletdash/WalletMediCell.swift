...
class WalletMediCell: UITableViewCell {
    ...
    func updateView(_ account: Account?, _ chainType: ChainType?) {
        let available = BaseData.instance.availableAmount(MEDI_MAIN_DENOM)
        let delegated = BaseData.instance.delegatedSumAmount()
        let unbonding = BaseData.instance.unbondingSumAmount()
        let reward = BaseData.instance.rewardAmount(MEDI_MAIN_DENOM)
        ...
        totalValue.attributedText = WUtils.dpUserCurrencyValue(MEDI_MAIN_DENOM, total, 6, totalValue.font)
