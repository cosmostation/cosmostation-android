    ...
    public void onClick(View v) {
        if(v.equals(mBeforeBtn)) {
            ...
            if (getSActivity().mBaseChain.equals(BaseChain.KAVA_MAIN) || getSActivity().mBaseChain.equals(BaseChain.KAVA_TEST)) {
                ...
                amount.add(gasCoin);
                fee.amount = amount;
                fee.gas = BaseConstant.KAVA_GAS_AMOUNT_CDP;
    ...
    private void onUpdateFeeLayer() {
        if (getSActivity().mBaseChain.equals(BaseChain.KAVA_MAIN) || getSActivity().mBaseChain.equals(BaseChain.KAVA_TEST)) {
            if(mSeekBarGas.getProgress() == 0) {
                ...
                mGasAmount.setText(BaseConstant.KAVA_GAS_AMOUNT_CDP);
                mGasRate.setText(WDp.getDpString(KAVA_GAS_RATE_LOW, 4));

                mFeeAmount = new BigDecimal(BaseConstant.KAVA_GAS_AMOUNT_CDP).multiply(new BigDecimal(KAVA_GAS_RATE_LOW)).setScale(0);
                if(getBaseDao().getCurrency() != 5) {
                    mFeePrice = WDp.uAtomToAtom(mFeeAmount).multiply(new BigDecimal(""+getBaseDao().getLastKavaTic())).setScale(2, RoundingMode.DOWN);
                } else {
                    mFeePrice = WDp.uAtomToAtom(mFeeAmount).multiply(new BigDecimal(""+getBaseDao().getLastKavaTic())).setScale(8, RoundingMode.DOWN);
                ...
                mFeeAmount = new BigDecimal(BaseConstant.KAVA_GAS_AMOUNT_CDP).multiply(new BigDecimal(KAVA_GAS_RATE_AVERAGE)).setScale(0);
                if(getBaseDao().getCurrency() != 5) {
                    mFeePrice = WDp.uAtomToAtom(mFeeAmount).multiply(new BigDecimal(""+getBaseDao().getLastKavaTic())).setScale(2, RoundingMode.DOWN);
                } else {
                    mFeePrice = WDp.uAtomToAtom(mFeeAmount).multiply(new BigDecimal(""+getBaseDao().getLastKavaTic())).setScale(8, RoundingMode.DOWN);
