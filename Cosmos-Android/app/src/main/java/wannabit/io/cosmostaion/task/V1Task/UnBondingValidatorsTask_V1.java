...
public class UnBondingValidatorsTask_V1 extends CommonTask {
    private BaseChain mChain;
    private int                     mOffset = 0;
    private boolean                 mBreak = false;
    private ArrayList<Validator_V1> resultData = new ArrayList<>();

    public UnBondingValidatorsTask_V1(BaseApplication app, TaskListener listener, BaseChain chain) {
        super(app, listener);
        this.mResult.taskType   = TASK_V1_FETCH_UNBONDING_VALIDATORS;
        this.mChain = chain;
    }
    ...
    private ArrayList<Validator_V1> onDoingJob(int offset) {
        ...
        try {
            if (mChain.equals(COSMOS_MAIN)) {
                ...
                Response<ResValidators_V1> response = ApiClient.getIrisChain(mApp).getUnBondingValidatorList(200,  offset).execute();
                if (response.isSuccessful()) {
                    if (response.body() != null && response.body().validators != null) {
                        resultData = response.body().validators;
                    }
                ...
                Response<ResValidators_V1> response = ApiClient.getAkashChain(mApp).getUnBondingValidatorList(200,  offset).execute();
                if (response.isSuccessful()) {
                    if (response.body() != null && response.body().validators != null) {
                        resultData = response.body().validators;
                    }
