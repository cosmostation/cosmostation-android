...
import static wannabit.io.cosmostaion.base.BaseConstant.BAND_GAS_AMOUNT_REDELEGATE;
...
import static wannabit.io.cosmostaion.base.BaseConstant.CERTIK_GAS_AMOUNT_REDELEGATE;
import static wannabit.io.cosmostaion.base.BaseConstant.CERTIK_GAS_RATE_AVERAGE;
import static wannabit.io.cosmostaion.base.BaseConstant.IOV_GAS_AMOUNT_REDELEGATE;
...
import static wannabit.io.cosmostaion.base.BaseConstant.KAVA_GAS_AMOUNT_REDELEGATE;
...
public class RedelegateStep3Fragment extends BaseFragment implements View.OnClickListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if (getSActivity().mBaseChain.equals(KAVA_MAIN) || getSActivity().mBaseChain.equals(KAVA_TEST)) {
            ...
            mFeeAmount = new BigDecimal(IOV_GAS_AMOUNT_REDELEGATE).multiply(new BigDecimal(IOV_GAS_RATE_AVERAGE)).setScale(0);
            ...
            mGasAmount.setText(CERTIK_GAS_AMOUNT_REDELEGATE);
            mGasRate.setText(WDp.getDpString(CERTIK_GAS_RATE_AVERAGE, 3));
            mFeeAmount = new BigDecimal(CERTIK_GAS_AMOUNT_REDELEGATE).multiply(new BigDecimal(CERTIK_GAS_RATE_AVERAGE)).setScale(0);
            if(getBaseDao().getCurrency() != 5) {
    ...
    public void onClick(View v) {
        if(v.equals(mBeforeBtn)) {
            ...
            if (getSActivity().mBaseChain.equals(KAVA_MAIN) || getSActivity().mBaseChain.equals(KAVA_TEST)) {
                ...
                amount.add(gasCoin);
                fee.amount = amount;
                fee.gas = KAVA_GAS_AMOUNT_REDELEGATE;
                ...
                amount.add(gasCoin);
                fee.amount = amount;
                fee.gas = BAND_GAS_AMOUNT_REDELEGATE;
                getSActivity().mTxFee = fee;
                ...
                amount.add(gasCoin);
                fee.amount = amount;
                fee.gas = IOV_GAS_AMOUNT_REDELEGATE;
                getSActivity().mTxFee = fee;
                ...
                amount.add(gasCoin);
                fee.amount = amount;
                fee.gas = CERTIK_GAS_AMOUNT_REDELEGATE;
                getSActivity().mTxFee = fee;
    ...
    private void onUpdateFeeLayer() {
        if (getSActivity().mBaseChain.equals(KAVA_MAIN) || getSActivity().mBaseChain.equals(KAVA_TEST)) {
            if(mSeekBarGas.getProgress() == 0) {
                ...
                mGasAmount.setText(KAVA_GAS_AMOUNT_REDELEGATE);
                mGasRate.setText(WDp.getDpString(KAVA_GAS_RATE_LOW, 4));

                mFeeAmount = new BigDecimal(KAVA_GAS_AMOUNT_REDELEGATE).multiply(new BigDecimal(KAVA_GAS_RATE_LOW)).setScale(0);
                if(getBaseDao().getCurrency() != 5) {
                    mFeePrice = WDp.uAtomToAtom(mFeeAmount).multiply(new BigDecimal(""+getBaseDao().getLastKavaTic())).setScale(2, RoundingMode.DOWN);
                } else {
                    mFeePrice = WDp.uAtomToAtom(mFeeAmount).multiply(new BigDecimal(""+getBaseDao().getLastKavaTic())).setScale(8, RoundingMode.DOWN);
                ...
                mFeeAmount = new BigDecimal(KAVA_GAS_AMOUNT_REDELEGATE).multiply(new BigDecimal(KAVA_GAS_RATE_AVERAGE)).setScale(0);
                if(getBaseDao().getCurrency() != 5) {
                    mFeePrice = WDp.uAtomToAtom(mFeeAmount).multiply(new BigDecimal(""+getBaseDao().getLastKavaTic())).setScale(2, RoundingMode.DOWN);
                } else {
                    mFeePrice = WDp.uAtomToAtom(mFeeAmount).multiply(new BigDecimal(""+getBaseDao().getLastKavaTic())).setScale(8, RoundingMode.DOWN);
            ...
            if(mSeekBarGas.getProgress() == 0) {
                ...
                mGasAmount.setText(BAND_GAS_AMOUNT_REDELEGATE);
                mGasRate.setText(WDp.getDpString(BAND_GAS_RATE_LOW, 4));

                mFeeAmount = new BigDecimal(BAND_GAS_AMOUNT_REDELEGATE).multiply(new BigDecimal(BAND_GAS_RATE_LOW)).setScale(0);
                if(getBaseDao().getCurrency() != 5) {
                    mFeePrice = WDp.uAtomToAtom(mFeeAmount).multiply(new BigDecimal(""+getBaseDao().getLastBandTic())).setScale(2, RoundingMode.DOWN);
                } else {
                    mFeePrice = WDp.uAtomToAtom(mFeeAmount).multiply(new BigDecimal(""+getBaseDao().getLastBandTic())).setScale(8, RoundingMode.DOWN);
                ...
                mFeeAmount = new BigDecimal(BAND_GAS_AMOUNT_REDELEGATE).multiply(new BigDecimal(BAND_GAS_RATE_AVERAGE)).setScale(0);
                if(getBaseDao().getCurrency() != 5) {
                    mFeePrice = WDp.uAtomToAtom(mFeeAmount).multiply(new BigDecimal(""+getBaseDao().getLastBandTic())).setScale(2, RoundingMode.DOWN);
                } else {
                    mFeePrice = WDp.uAtomToAtom(mFeeAmount).multiply(new BigDecimal(""+getBaseDao().getLastBandTic())).setScale(8, RoundingMode.DOWN);
