...
class CancelUnBondingFragment(
    ...
    private fun initView() {
        binding.apply {
            cancelUnstakingView.setBackgroundResource(R.drawable.cell_bg)
            memoView.setBackgroundResource(R.drawable.cell_bg)
            feeView.setBackgroundResource(R.drawable.cell_bg)

            selectedChain.cosmosValidators.firstOrNull { it.operatorAddress == unBondingEntry?.validatorAddress }?.let { validator ->
            ...
            selectedChain.stakeDenom?.let { denom ->
                BaseData.getAsset(selectedChain.apiName, denom)?.let { asset ->
                    asset.decimals?.let { decimal ->
                        val unbondingAmount = unBondingEntry?.entry?.balance?.toBigDecimal()?.movePointLeft(decimal) ?: BigDecimal.ZERO
                        cancelAmount.text = formatAmount(unbondingAmount.toPlainString(), decimal)
        ...
        binding.apply {
            ...
            btnCancelUnstake.setOnClickListener {
                Intent(requireContext(), PasswordCheckActivity::class.java).apply {
                    getCompoundingResultLauncher.launch(this)
    ...
    private val getCompoundingResultLauncher: ActivityResultLauncher<Intent> =
        registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
            if (result.resultCode == Activity.RESULT_OK && isAdded) {
                binding.backdropLayout.visibility = View.VISIBLE
                txViewModel.broadCancelUnbonding(getChannel(selectedChain), selectedChain.address, onBindCancelUnbonding(), txFee, txMemo, selectedChain)
            }
        }
    ...
    private fun onBindCancelUnbonding(): MsgCancelUnbondingDelegation? {
        val toCoin =
            CoinProto.Coin.newBuilder().setDenom(selectedChain.stakeDenom).setAmount(unBondingEntry?.entry?.balance).build()
