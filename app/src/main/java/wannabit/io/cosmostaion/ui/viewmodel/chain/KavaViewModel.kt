...
class KavaViewModel(private val kavaRepository: KavaRepository) : ViewModel() {
    ...
    fun lendingData(managedChannel: ManagedChannel, address: String?) =
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                val responses = awaitAll(lendingParamDeferred, interestRateDeferred, totalDepositDeferred, totalBorrowDeferred, myDepositDeferred, myBorrowDeferred, reserveDeferred)
                ...
                responses.forEach { response ->
                    when (response) {
                        is NetworkResult.Success -> {
                            ...
                            _lendingData.postValue(LendingData(lendingParam, lendingRates, lendingTotalDeposits, lendingTotalBorrows, lendingMyDeposits, lendingMyBorrows, lendingReserve))
                        }
    ...
    fun swapData(managedChannel: ManagedChannel, address: String?) =
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                val swapMyDepositDeferred = async { kavaRepository.swapMyDeposit(managedChannel, address) }

                val responses = awaitAll(swapListDeferred, swapMyDepositDeferred)

                var swapPools: MutableList<com.kava.swap.v1beta1.QueryProto.PoolResponse>? = mutableListOf()
                var swapMyDeposits: MutableList<com.kava.swap.v1beta1.QueryProto.DepositResponse>? = mutableListOf()
    ...
    fun bep3Data(managedChannel: ManagedChannel) = CoroutineScope(Dispatchers.IO).launch {
        try {
    ...
    fun bep3SwapId(managedChannel: ManagedChannel, expectedSwapId: String?, toChain: CosmosLine?) = CoroutineScope(Dispatchers.IO).launch {
        if (toChain is ChainBinanceBeacon) {
            when (val response = safeApiCall { RetrofitInstance.beaconApi.swapById(expectedSwapId) }) {
                is NetworkResult.Success -> {
            ...
            try {
                when (val response = kavaRepository.bep3SwapId(managedChannel, expectedSwapId)) {
                    is NetworkResult.Success -> {
    ...
    fun accountInfo(address: String?) = CoroutineScope(Dispatchers.IO).launch {
