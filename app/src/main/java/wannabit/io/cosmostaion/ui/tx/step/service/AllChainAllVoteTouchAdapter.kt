...
class AllChainAllVoteTouchAdapter(
    private val allDisplayInfos: MutableList<VoteAllModel>,
    ...
    private fun getSectionPosition(adapterPosition: Int): Int {
        ...
        for (index in allDisplayInfos.indices) {
            if (adapterPosition < itemCountSoFar + 1 + allDisplayInfos[index].proposals.size) {
                return index
            }
            itemCountSoFar += 1 + allDisplayInfos[index].proposals.size
        }
    ...
    private fun getHideView(viewHolder: RecyclerView.ViewHolder): View {
        return (viewHolder as AllChainAllVoteViewHolder).itemView.findViewById(R.id.delete_layout)
    }
