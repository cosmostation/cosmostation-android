...
open class CosmosFetcher(private val chain: BaseChain) {
    ...
    fun rewardOtherDenoms(): Int {
        return rewardAllCoins().map { it.denom }.distinct().count { it != chain.getStakeAssetDenom() }
    }
    ...
    fun compoundAbleRewards(): MutableList<DistributionProto.DelegationDelegatorReward?> {
        val result: MutableList<DistributionProto.DelegationDelegatorReward?> = mutableListOf()
        cosmosRewards.forEach { reward ->
            reward.rewardList.firstOrNull { it.denom == chain.getStakeAssetDenom() }?.amount?.let { amount ->
                val rewardAmount =
    ...
    open fun allStakingDenomAmount(): BigDecimal? {
        return balanceAmount(chain.getStakeAssetDenom()).add(delegationAmountSum())?.add(unbondingAmountSum())
            ?.add(rewardAmountSum(chain.getStakeAssetDenom()))
    }

    fun delegateAbleAmount(): BigDecimal {
        return if (chain is ChainCoreum) {
            availableAmount(chain.getStakeAssetDenom())
        } else {
            balanceAmount(chain.getStakeAssetDenom())
        }
    }
