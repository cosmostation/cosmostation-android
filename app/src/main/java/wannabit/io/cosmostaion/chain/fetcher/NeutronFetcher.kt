...
class NeutronFetcher(private val chain: BaseChain) : CosmosFetcher(chain) {
    ...
    override fun allStakingDenomAmount(): BigDecimal {
        return chain.cosmosFetcher?.balanceAmount(chain.getStakeAssetDenom())?.add(neutronVestingAmount())
            ?.add(delegationValueSum())?.add(unbondingValueSum())
            ?.add(rewardAmountSum(chain.getStakeAssetDenom()))?.add(neutronDeposited)?.add(neutronRewards)
            ?: BigDecimal.ZERO
    }
