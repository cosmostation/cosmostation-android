...
import static wannabit.io.cosmostaion.base.BaseConstant.COINGECKO_ASSETMANTLE_MAIN;
...
public class AssetMantle extends Chain {
    ...
    public void setMainIntent(MainActivity mainActivity, int sequence) {
        if (sequence == 1) {
            mainActivity.startActivity(new Intent(Intent.ACTION_VIEW , Uri.parse(COINGECKO_ASSETMANTLE_MAIN)));
        } else if (sequence == 2) {
    ...
    @Override
    public HistoryApi getHistoryApi(Context c) { return ApiClient.getMantleApi(c); }
}
