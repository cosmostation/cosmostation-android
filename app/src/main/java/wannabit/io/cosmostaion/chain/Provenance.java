...
import static wannabit.io.cosmostaion.base.BaseConstant.COINGECKO_PROVENANCE_MAIN;
...
public class Provenance extends Chain {
    ...
    public void setMainIntent(MainActivity mainActivity, int sequence) {
        if (sequence == 1) {
            mainActivity.startActivity(new Intent(Intent.ACTION_VIEW , Uri.parse(COINGECKO_PROVENANCE_MAIN)));
        } else if (sequence == 2) {
