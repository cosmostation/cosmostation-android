...
public class DepositPoolActivity extends BaseBroadCastActivity {
    ...
    public void onStartJoinPool() {
        if (getBaseDao().isAutoPass()) {
            ServiceOuterClass.BroadcastTxRequest broadcastTxRequest = Signer.getGrpcKavaDepositReq(getAuthResponse(mBaseChain, mAccount), mAccount.address, mKavaPoolTokenA, mKavaPoolTokenB,
                    BaseConstant.KAVA_SLIPPAGE, (System.currentTimeMillis() / 1000) + 300, mTxFee, mTxMemo, getEcKey(mAccount), getBaseDao().getChainIdGrpc());
