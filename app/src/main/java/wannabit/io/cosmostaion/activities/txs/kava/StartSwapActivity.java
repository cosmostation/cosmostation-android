...
public class StartSwapActivity extends BaseBroadCastActivity {
    ...
    private void onBroadCastTx() {
        new KavaSwapGrpcTask(getBaseApplication(), new TaskListener() {
            @Override
            public void onTaskResponse(TaskResult result) {
                if (result.isSuccess) {
                    Intent txIntent = new Intent(StartSwapActivity.this, TxDetailgRPCActivity.class);
        ...
        }, mAccount, mBaseChain, mAccount.address, mKavaSwapIn, mKavaSwapOut, mTxMemo, mTxFee, getBaseDao().getChainIdGrpc()).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
    }
    ...
    private class CoinSwapPageAdapter extends FragmentPagerAdapter {
        ...
        public CoinSwapPageAdapter(FragmentManager fm) {
            super(fm);
            mFragments.clear();
            mFragments.add(KavaSwapStep0Fragment.newInstance());
            mFragments.add(StepMemoFragment.newInstance());
            mFragments.add(StepFeeSetFragment.newInstance());
            mFragments.add(KavaSwapStep3Fragment.newInstance());
        }
