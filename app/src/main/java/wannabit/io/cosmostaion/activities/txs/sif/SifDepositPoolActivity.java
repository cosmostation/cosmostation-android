...
public class SifDepositPoolActivity extends BaseBroadCastActivity {
    ...
    public void onStartJoinPool() {
        if (getBaseDao().isAutoPass()) {
            ServiceOuterClass.BroadcastTxRequest broadcastTxRequest = Signer.getGrpcSifDepositReq(getAuthResponse(mBaseChain, mAccount), mAccount.address, mSifDepositCoin1.denom,
                    mSifDepositCoin0.amount, mSifDepositCoin1.amount, mTxFee, mTxMemo, getEcKey(mAccount), getBaseDao().getChainIdGrpc());
