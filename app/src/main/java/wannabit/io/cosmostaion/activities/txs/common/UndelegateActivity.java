...
public class UndelegateActivity extends BaseBroadCastActivity {
    ...
    private void onUndelegateByLedger() {
        runOnUiThread(() -> LedgerManager.getInstance().signAndBroadcast(UndelegateActivity.this, mAccount, new LedgerManager.LedgerSignListener() {
            @NonNull
            @Override
            public String getMessage() {
                ArrayList<Msg> undelegateMsgs = MsgGenerator.genUndelegateMsgs(mAccount.address, mValAddress, mAmount);
                return WKey.onGetLedgerMessage(getBaseDao(), mChainConfig, mAccount, undelegateMsgs, mTxFee, mTxMemo);
            }

            @NonNull
            @Override
            public ServiceOuterClass.BroadcastTxRequest makeBroadcastTxRequest(@NonNull byte[] currentPubKey) {
                return Signer.getGrpcLedgerUnDelegateReq(WKey.onAuthResponse(mBaseChain, mAccount), mValAddress, mAmount, mTxFee, mTxMemo, LedgerManager.Companion.getInstance().getCurrentPubKey(), WKey.getLedgerSigData(currentPubKey));
            }
