...
class DappFragment : BaseFragment() {
    ...
    override fun onRefreshTab() {
        ...
        val baseUrl = if (BuildConfig.DEBUG) {
            "https://dapps.dev.cosmostation.io?chain="
        } else {
            "https://dapps.cosmostation.io?chain="
        }
        webView.loadUrl(baseUrl + baseActivity.mChainConfig.chainName() + "&theme=" + ThemeUtil.currentMode(context))
    }
    ...
    private fun initWebView() {
        ...
        webView.settings.userAgentString = webView.settings.userAgentString + " Cosmostation/APP/Android/DappTab" + BuildConfig.VERSION_NAME
                ...
                (activity as? MainActivity)?.let {
                    if (it.mAccount.isLedger) {
                        CommonAlertDialog.showSingleButton(
                            it, it.getString(R.string.str_with_ledger), it.getString(R.string.str_ledger_not_support_dapp_msg), it.getString(R.string.str_confirm), null, true
                        )
                        return true

                    } else if (!it.mAccount.hasPrivateKey) {
                        CommonAlertDialog.showDoubleButton(
                            it, it.getString(R.string.str_only_observe_title), it.getString(R.string.str_only_observe_msg), Html.fromHtml("<font color=\"#9C6CFF\">" + it.getString(R.string.str_add_mnemonics) + "</font>", Html.FROM_HTML_MODE_COMPACT), { view: View? -> it.onAddMnemonicForAccount() }, it.getString(R.string.str_close), null
                        )
                        return true
                    }
                }
