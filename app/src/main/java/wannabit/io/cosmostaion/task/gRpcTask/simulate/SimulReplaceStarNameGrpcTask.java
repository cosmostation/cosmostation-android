...
public class SimulReplaceStarNameGrpcTask  extends CommonTask {
    ...
    protected TaskResult doInBackground(String... strings) {
        try {
            ...
            ServiceOuterClass.SimulateRequest simulateTxRequest = Signer.getGrpcReplaceResourceSimulateReq(WKey.onAuthResponse(mBaseChain, mAccount), mDomain, mName, mAccount.address, mResources, mFees, mMemo, WKey.getECKey(mApp, mAccount), mChainId, mAccount.customPath, mBaseChain);
            ServiceOuterClass.SimulateResponse response = txService.simulate(simulateTxRequest);
            mResult.resultData = response.getGasInfo();
            mResult.isSuccess = true;

        } catch (Exception e) {
            WLog.e( "SimulReplaceStarNameGrpcTask "+ e.getMessage());
